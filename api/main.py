...
def read_one_sandwich(sandwich_id: int, db: Session = Depends(get_db)):
    sandwich = sandwiches.read_one_sandwich(db, sandwich_id)
    if sandwich is None:
...
def read_one_resource(resource_id: int, db: Session = Depends(get_db)):
    return resources.read_one_resource(db, resource_id)
...
def read_one_recipe(recipe_id: int, db: Session = Depends(get_db)):
    return recipes.read_one_recipe(db, recipe_id)
...
def read_one_order_detail(order_detail_id: int, db: Session = Depends(get_db)):
    return order_details.read_one_order_detail(db, order_detail_id)
